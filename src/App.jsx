...
const WISE_AFFILIATE_URL = "https://wise.prf.hn/click/camref:1100l5J9wt";

const PROVIDERS = [
  { id: 1, name: "Wise", logo: "🦉", rating: 4.8, reviews: "142K", speed: "1-2 hrs", color: "#9FE870", url: WISE_AFFILIATE_URL },
  { id: 2, name: "Remitly", logo: "💸", rating: 4.6, reviews: "89K", speed: "Minutes", color: "#1B75BC", url: "#" },
  { id: 3, name: "Western Union", logo: "🟡", rating: 4.1, reviews: "67K", speed: "Minutes", color: "#FFDD00", url: "#" },
  { id: 4, name: "Xoom (PayPal)", logo: "🅿️", rating: 4.3, reviews: "34K", speed: "1-3 days", color: "#0070BA", url: "#" },
  { id: 5, name: "WorldRemit", logo: "🌍", rating: 4.5, reviews: "52K", speed: "Minutes", color: "#7B2D8E", url: "#" },
  { id: 6, name: "OFX", logo: "💱", rating: 4.4, reviews: "28K", speed: "1-2 days", color: "#00264D", url: "#" },
  { id: 7, name: "MoneyGram", logo: "🔵", rating: 4.0, reviews: "45K", speed: "Minutes", color: "#FF6600", url: "#" },
  { id: 8, name: "Instarem", logo: "⚡", rating: 4.5, reviews: "19K", speed: "1 day", color: "#6C5CE7", url: "#" },
];
...
// ─── Provider Logo/Emoji Map (fallback when API doesn't return logos) ────────

const PROVIDER_META = {
  wise:           { logo: "🦉", emoji: "🦉", rating: 4.8, reviews: "142K", url: WISE_AFFILIATE_URL },
  remitly:        { logo: "💸", emoji: "💸", rating: 4.6, reviews: "89K", url: "#" },
  westernunion:   { logo: "🟡", emoji: "🟡", rating: 4.1, reviews: "67K", url: "#" },
  "western union":{ logo: "🟡", emoji: "🟡", rating: 4.1, reviews: "67K", url: "#" },
  xoom:           { logo: "🅿️", emoji: "🅿️", rating: 4.3, reviews: "34K", url: "#" },
  paypal:         { logo: "🅿️", emoji: "🅿️", rating: 4.3, reviews: "34K", url: "#" },
  worldremit:     { logo: "🌍", emoji: "🌍", rating: 4.5, reviews: "52K", url: "#" },
  ofx:            { logo: "💱", emoji: "💱", rating: 4.4, reviews: "28K", url: "#" },
  moneygram:      { logo: "🔵", emoji: "🔵", rating: 4.0, reviews: "45K", url: "#" },
  instarem:       { logo: "⚡", emoji: "⚡", rating: 4.5, reviews: "19K", url: "#" },
  currenciesdirect:{ logo: "💷", emoji: "💷", rating: 4.3, reviews: "12K", url: "#" },
  xe:             { logo: "🔷", emoji: "🔷", rating: 4.2, reviews: "15K", url: "#" },
  default:        { logo: "🏦", emoji: "🏦", rating: 3.8, reviews: "N/A", url: "#" },
};

function getProviderMeta(name) {
  const key = (name || "").toLowerCase().replace(/[^a-z]/g, "");
  return PROVIDER_META[key] || PROVIDER_META[(name || "").toLowerCase()] || PROVIDER_META.default;
}
...
// ─── Wise Comparison API Hook ────────────────────────────────────────────────

function useWiseComparison(amount, from, to) {
  ...
  const fetchComparison = useCallback(async () => {
    ...
    try {
      ...
      // Transform API providers into our component format
      const providers = (json.providers || []).map((p, i) => {
        ...
        return {
          ...
          speed: p.formattedDelivery || "Unknown",
          rate: +(p.rate || 0).toFixed(4),
          fee: +(p.fee || 0).toFixed(2),
          received: +(p.receivedAmount || 0).toFixed(2),
          total: +(p.sourceAmount || amount).toFixed(2),
          isMidMarketRate: p.isMidMarketRate || false,
          isPartner: p.isPartner || false,
          url: p.name.toLowerCase().includes("wise") ? WISE_AFFILIATE_URL : (meta.url || "#"),
      ...
      setData({
        providers,
        midMarketRate: json.midMarketRate,
        maxSavings: json.maxSavings,
        bestProvider: json.bestProvider,
        worstProvider: json.worstProvider,
        providerCount: json.providerCount,
        lastUpdated: json.lastUpdated,
      });
      setLastFetched(new Date());
    } catch (err) {
      console.warn("Wise API unavailable, using mock data:", err.message);
  ...
  useEffect(() => {
    ...
    timeoutRef.current = setTimeout(fetchComparison, 500);
  ...
  return { data, loading, error, lastFetched, refetch: fetchComparison };
}

// ─── Wise Live Rate Hook ─────────────────────────────────────────────────────

function useWiseRate(from, to) {
  ...
  useEffect(() => {
    ...
    async function fetchRate() {
      try {
        const res = await fetch(`/api/rates?source=${from}&target=${to}`);
        if (res.ok) {
          const json = await res.json();
          if (!cancelled && json.rate) setLiveRate(json.rate);
        }
      } catch {
        // Silently fall back to static rate
      }
    }
...
function Hero({ setCurrentPage }) {
  ...
  const wiseRate = useWiseRate(from, to);
  const effectiveRate = wiseRate || getRate(from, to);
            ...
            <div className="relative bg-gray-900/80 backdrop-blur-xl border border-white/10 rounded-3xl p-6 sm:p-8 shadow-2xl">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between px-2 text-sm">
                  ...
                  <span className="text-white font-semibold">1 {from} = {effectiveRate.toFixed(4)} {to}</span>
                </div>
...
function CompareRates() {
  ...
  // Try live Wise Comparison API first, fall back to mock data
  const { data: liveData, loading: liveLoading, error: liveError, lastFetched, refetch } = useWiseComparison(amount, from, to);

  const isLive = liveData && liveData.providers && liveData.providers.length > 0;
  const providers = isLive ? liveData.providers : generateProviderRates(amount, from, to);
  const best = providers[0];
  const worst = providers[providers.length - 1];
  const savings = isLive
    ? (liveData.maxSavings || 0).toFixed(2)
    : (best.received - worst.received).toFixed(2);
        ...
        <div className="bg-gray-900/50 border border-white/10 rounded-2xl p-4 sm:p-6 mb-8">
          <div className="flex flex-wrap items-end gap-4">
            ...
            <button
              ...
              className="px-6 py-3 bg-gradient-to-r from-emerald-400 to-cyan-400 text-gray-900 font-bold rounded-xl flex items-center gap-2 hover:shadow-lg hover:shadow-emerald-400/25 transition-all self-end disabled:opacity-50"
            >
              <RefreshCw className={cn("w-4 h-4", liveLoading && "animate-spin")} /> {liveLoading ? "Loading..." : "Refresh"}
            </button>
          </div>
          ...
          <div className="flex items-center gap-3 mt-4">
            <div className={cn("flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium border",
              isLive ? "bg-emerald-400/10 border-emerald-400/30 text-emerald-400" : "bg-yellow-400/10 border-yellow-400/30 text-yellow-400"
            )}>
              <div className={cn("w-1.5 h-1.5 rounded-full", isLive ? "bg-emerald-400 animate-pulse" : "bg-yellow-400")} />
              {isLive ? "Live from Wise API" : "Estimated rates (demo)"}
            </div>
            {lastFetched && (
            ...
            {liveError && (
              <span className="text-xs text-gray-500 flex items-center gap-1">
                <Info className="w-3 h-3" /> API unavailable — showing estimates
              </span>
            )}
          </div>
        </div>
            ...
            >
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                <div className="flex items-center gap-4 min-w-[200px]">
                  ...
                  <div>
                    {p.url && p.url !== "#" ? (
                      <a href={p.url} target="_blank" rel="noopener noreferrer" className="text-white font-bold hover:text-emerald-400 transition-colors">{p.name}</a>
                    ) : (
                ...
                <a
                  ...
                  className={cn(
                    "px-6 py-3 rounded-xl font-bold text-sm transition-all whitespace-nowrap text-center inline-block",
                    i === 0
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="text-white font-bold">
                        {p.name}
                        {p.url && p.url !== "#" && <span className="ml-2 text-xs text-emerald-400 font-normal">Partner</span>}
                      </div>
          ...
          {step === 5 && (
            <div className="space-y-6">
              ...
              <div className="flex gap-3">
                ...
                <a
                  href={selectedProvider?.url || WISE_AFFILIATE_URL}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 py-4 bg-gradient-to-r from-emerald-400 to-cyan-400 text-gray-900 font-bold rounded-2xl hover:shadow-lg transition-all flex items-center justify-center gap-2"
                >
                ...
                {TRANSACTIONS.map((tx) => (
                  <tr key={tx.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    ...
                    <td className="py-4 pr-4 text-sm">
                      {tx.provider === "Wise" ? (
                        <a href={WISE_AFFILIATE_URL} target="_blank" rel="noopener noreferrer" className="text-emerald-400 hover:text-emerald-300 font-medium underline underline-offset-2">{tx.provider}</a>
                      ) : (
                        <span className="text-gray-300">{tx.provider}</span>
                      )}
                    </td>
                    <td className="py-4">
                      <span className={cn(
